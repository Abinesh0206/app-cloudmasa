...
resource "aws_route53_record" "record" {
  ...
  # Alias for ALB (e.g., *.elb.amazonaws.com)
  dynamic "alias" {
    for_each = local.is_alb_target && var.record_type == "A" ? [1] : []
    content {
      name                   = "${var.target}."
      zone_id                = "Z35SXDOTRQ7X7K"  # Global ELB zone ID
      evaluate_target_health = true
    }
  }

  # ONLY set records if NOT using alias
  records = local.is_alb_target && var.record_type == "A" ? null : [var.target]

  # TTL: required for non-alias; not allowed for alias
